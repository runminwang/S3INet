...
class TextLoss(nn.Module):
    ...
    def cls_ohem(self, predict, target, train_mask, negative_ratio=3.):
        ...
        n_pos = pos.float().sum()
        if n_pos.item() > 0:
            loss_pos = self.BCE_loss(predict[pos], target[pos].float()).sum()
            loss_neg = self.BCE_loss(predict[neg], target[neg].float())
    ...
    #       calculate textsnake loss
    #     """
    #     # print("train_expand_mask is {}".format(train_expand_mask))
    #     # print("train_mask is {}".format(train_mask))
    #     # print("tr_mask is {}".format(tr_mask))
    #
    #
    #     b, c, h, w = inputs.shape
    #     # print(f"inputs.shape is {b},{c},{h},{w}")
    ...
    #     # 分割图损失
    #     # seg_loss = self.dice_loss(attention, expand_mask, train_expand_mask)
    #     seg_loss = self.cls_ohem(attention, expand_mask, train_expand_mask)
    #
    #
    #     # # 原有损失函数权重
    #     # config.alpha = 1
