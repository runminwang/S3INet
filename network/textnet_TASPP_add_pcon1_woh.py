...
# 消融改进的TASPP,对文本特征处理部分使用单一方向的文本特征表示（单独垂直方向）
import torch
...
class horizontal_Dilate(nn.Module):  # in_channel 为各个小分支通道大小
    def __init__(self, in_channel, out_channel, output_stride=16):
        ...
        # 仅使用垂直方向卷积进行文本特征提取
        self.fca2 = nn.Conv2d(in_channel, in_channel, kernel_size=(3,1), padding=(dilations[0], 0),
            dilation=dilations[0], bias=False)

        self.fca3 = nn.Conv2d(in_channel, in_channel, kernel_size=(5,1), padding=(2*dilations[1], 0),
            dilation=dilations[1], bias=False)

        self.fca4 = nn.Conv2d(in_channel, in_channel, kernel_size=(7,1), padding=(3*dilations[2], 0),
            dilation=dilations[2], bias=False)
