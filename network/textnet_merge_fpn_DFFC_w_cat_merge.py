# 消融实验：针对经过多尺度语义信息融合之后的底层特征，使用cat融合作为底层特征的融合方式，不使用多分支并行融合
...
class Context_Semantics(nn.Module):
    ...
    def forward(self, x):
        ...
        # 直接使用cat融合方式，不使用并行交互融合
        p2 = F.upsample(p2, size=(x0_h, x0_w), mode='bilinear')
        p3 = F.upsample(p3, size=(x0_h, x0_w), mode='bilinear')


        # cat融合,使用3*3深度可分离卷积降维块处理
        low_feature = self.ConvLinear2(torch.cat([p1, p2, p3], 1))
        ...
        # 因为要在训练过程中对其进行损失计算，所以返回相应注意力图，exp（）加大前景与背景的差距（不对其使用exp计算）
        # 具体来说，unsqueeze(1)将维度从(batch_size, height, width)扩展为(batch_size, 1, height, width)，
        attention = text_prob[:, 1, :, :].unsqueeze(1)  # attention.shape is torch.Size([3, 1, 200, 304])
...
class FPN(nn.Module):
    ...
    def forward(self, x):
        ...
        return feature_total, attention
