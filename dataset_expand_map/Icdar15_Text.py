...
# ctst、TPD运行使用
    ...
    #
    #
    #         # ctst中只有8个点坐标，默认取最后一个数作为label
    #         label = gt[-1].strip().replace("###", "#")
    #         # print(f"label is {label}")  # 最后一个坐标值或者text
    #
    #         pts = np.stack([xx, yy]).T.astype(np.int32)
    #         # d1 = norm2(pts[0] - pts[1])
    #         # d2 = norm2(pts[1] - pts[2])
    ...
    def __getitem__(self, item):
        ...
        try:
            ...
            print("Read annotation Exception is {}".format(e))
            polygons = None

        # return self.get_training_data(image, polygons, image_id=image_id, image_path=image_path)
        return self.get_training_data(image, polygons, image_id=image_id, image_path=image_path, ratio=cfg.ratio)
