...
class TextDataset(object):
    ...
    def get_expand_map(self, polygon, expand_mask, train_expand_mask, ratio):
        ...
        # 对每个polygon进行shrunk
            ...
            # assert polygon.shape[1] == 2  # 每个点2个数x,y
            # #
            # # 多边形面积、周长
            # # area = polygon_shape.area
            # # print("polygon_shape.area is {}".format(area))
            # length = polygon_shape.length
            #
            # # 关于距离D有无约束？（先使用8进行训练，之后进行消融）
            # distance = min((int)(area * (1 - np.power(ratio, 2)) / length),30)# 计算论文中的D
            # # distance = area * (1 - np.power(ratio, 2)) / length # 计算论文中的D
            # # print(f"shrunk_D is {distance}")
            # subject = [tuple(l) for l in polygon]
            # padding = pyclipper.PyclipperOffset()
            # padding.AddPath(subject, pyclipper.JT_ROUND,
            #                 pyclipper.ET_CLOSEDPOLYGON)
            #
            # # padded_polygon = np.array(padding.Execute(distance)[0])  # 正的distance就是膨胀，向外膨胀distance个距离
            # # cv2.fillPoly(mask, [padded_polygon.astype(np.int32)], 1.0)  # 膨胀之后mask填充1
            #
            # # 正的distance就是膨胀，向外膨胀distance个距离
            # padded_polygon = np.array(padding.Execute(-distance)[0])
            #
            # # 如果为收缩，则对原多边形该区域进行全零掩码
            # if padded_polygon.size == 0:
            #     cv2.fillPoly(train_expand_mask, [polygon], color=(0,))
            # # 之后是否需要对膨胀文本实力图进行判断？
            #
            # cv2.fillPoly(expand_mask, [padded_polygon.astype(np.int32)], color=(1,))  # 膨胀之后mask填充1n
