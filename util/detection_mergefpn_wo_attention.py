...
def watershed_segment(preds, scale=1.0):
    ...
    # cv2.getStructuringElement返回指定形状和尺寸的内核
    ...
    ret, m = cv2.connectedComponents(kernal)# 得到不同前景的数目、编号

    distance = np.mean(preds[:], axis=0)
    distance = np.array(distance / np.max(distance) * 255, dtype=np.uint8)
    labels = segmentation.watershed(-distance, m, mask=region)  # 得到收缩（更加精细化）不同前景的编号
    boxes = []
    contours = []
    ...
    for idx in range(1, np.max(labels) + 1):
        ...
        # 如果用的是cv4.0版本，findContours返回的是两个参数，旧版的返回的则是三个参数
        # _, cont, _ = cv2.findContours(text_mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    def detect(self, image, img_show=None):
        ...
        with torch.no_grad():
            ...
            if cfg.exp_name != "Icdar2015":
                ...
                # preds, attention, backbone_time, iter_time = self.model.forward(img)

            else:
                preds, backbone_time, iter_time = self.model.forward(image)
                # preds, attention, backbone_time, iter_time = self.model.forward(image)
