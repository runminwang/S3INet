...
def default_evaluation_params():
    ...
    return {
        ...
        # 根据TPD命名，修改正则化表示
        # 'GT_SAMPLE_NAME_2_ID':'gt_Test_([0-9]+).txt',
...
def validate_data(gtFilePath, submFilePath, evaluationParams):
    ...
    for k in gt:
        ...
        rrc_evaluation_funcs.validate_lines_in_file(k, gt[k], evaluationParams['CRLF'], evaluationParams['LTRB'], False, False)
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    with open(fid_path, 'w') as f:
        for resFile in tqdm(gt):
            ...
            # 修改
            pointsList, _, _ = rrc_evaluation_funcs.get_tl_line_values_from_file_contents(gtFile,
                                                                                                           evaluationParams[
                                                                                                               'CRLF'],
                                                                                                           evaluationParams[
                                                                                                               'LTRB'],
