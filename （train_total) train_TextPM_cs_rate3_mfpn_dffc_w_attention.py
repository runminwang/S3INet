...
# 使用辅助损失的损失
from network.loss_sub_w_attention import TextLoss
...
# 加载自己预训练完成的模型进行微调（全部完全严格加载默认值strict=True)
...
def train(model, train_loader, criterion, scheduler, optimizer, epoch, device, logger=None):
    ...
    for i, (img, train_mask, tr_mask, expand_mask, train_expand_mask) in enumerate(train_loader):
        ...
        loss, aux_loss = criterion(output, attention, train_mask, tr_mask, expand_mask, train_expand_mask)
        ...
        if i % cfg.display_freq == 0:
            print('({:d} / {:d})  TotalLoss: {:.4f} AuxLoss: {:.4f}'
                  .format(i, len(train_loader), loss.item(), aux_loss.item()))
    ...
    print('Training Loss: {}\t Segloss: {}\t Regloss: {}'.format(losses.avg, losses_seg.avg,
                                                                 (losses.avg - losses_seg.avg)))
